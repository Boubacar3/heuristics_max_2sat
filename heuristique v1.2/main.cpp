    ...
    for (const auto& clause : clauses) {
        ...
        if (clause.literals.size() == 1) {
            ...
            // Case 2: Both literals are positive (x OR y)
                ...
                reduced_formula.push_back({-w, {-x, y}});   // Mixed clause
                reduced_formula.push_back({2.0 * w, {x}});  // Unit clause
            ...
            // Case 3: Both literals are negative (!x OR !y)
                ...
                reduced_formula.push_back({2.0 * w, {-u}}); // Unit clause
...
// --- Main Execution ---
int main() {
    string input_folder = "./test_wcnf_files_20000_var_100000_c";   // Change to your folder path
    string output_csv = "results_random_v1.2.csv";      // The desired output CSV name
