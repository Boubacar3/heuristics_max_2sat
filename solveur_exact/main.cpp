...
map<int, bool> solve_max2sat_implications_ilp(const vector<Clause>& wcnf_array, int timeout_seconds) {
    IloEnv env;
...
int export_linear_relaxation_to_lp(const vector<Clause>& wcnf_array, const string& output_file_path) {
    IloEnv env;
    int max_var = get_max_var(wcnf_array);
    int result = std::numeric_limits<double>::quiet_NaN();
    int objective_value = std::numeric_limits<double>::quiet_NaN();
...
void process_folder_to_csv(const string& folder_path, const string& csv_path, int timeout_seconds, bool export_lp = true) {
    ...
    for (const auto& entry : fs::directory_iterator(folder_path)) {
        if (entry.is_regular_file() && entry.path().extension() == ".wcnf") {
            ...
            int linear_relaxation_objective = std::numeric_limits<int>::quiet_NaN();
            ...
            map<int, bool> assignment = solve_max2sat_implications_ilp(reduced_clauses, timeout_seconds);
...
int main(int argc, char* argv[]) {
    ...
    int timeout_seconds; // Default CPLEX time limit in seconds
